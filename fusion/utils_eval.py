...
def calculate_similarity(df):
    ...
    # Explicitly disable weights-only loading to allow RDKit fingerprint objects
    # in the cached novelty file to be unpickled under PyTorch >=2.6.
    train_smiles, train_fps = torch.load(
        os.path.join(ROOT_DIR, 'data/zinc250k_novelty.pt'),
        weights_only=False,
    )
