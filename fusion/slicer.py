...
class MolSlicer:
    ...
    def __call__(self, mol):
        try:
            ...
            # RDKit may return None for invalid SMILES; bail out early to avoid native crashes
            if mol is None:
            ...
            # any RDKit failure -> skip this molecule
            return (None, None, None)
        ...
        if len(selected_bonds) == 1:
            # there is no linker
            tmp = Chem.rdmolops.FragmentOnBonds(mol, [selected_bonds[0].GetIdx()])
        ...
        bond_indices = [selected_bonds[c].GetIdx() for c in choice]
        # remove potential duplicates; RDKit FragmentOnBonds can segfault on repeated bond ids
        bond_indices = list(dict.fromkeys(bond_indices))
        if len(bond_indices) == 1:
            tmp = Chem.rdmolops.FragmentOnBonds(mol, bond_indices)
            head, tail = Chem.GetMolFrags(tmp, asMols=True)
            return (head, None, tail)
        selected_bonds = [mol.GetBondWithIdx(idx) for idx in bond_indices]
