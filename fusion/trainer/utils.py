...
class _NoSaveTokenizerWrapper:
    """
    包装原始 tokenizer，屏蔽 save_pretrained 以避免
    \"Custom PreTokenizer cannot be serialized\" 报错。
    训练/推理阶段的其它行为全部透传给底层 tokenizer。
    """
    ...
    def save_pretrained(self, *args, **kwargs):
        # SAFETokenizer 使用自定义 PreTokenizer，无法通过 HuggingFace 的
        # save_pretrained 序列化；这里直接跳过保存。
        return []

    def __call__(self, *args, **kwargs):
        # 转发所有调用行为（例如 tokenizer('.')）
...
class SAFEFusionCollator(SAFECollator):
    def __init__(self, **kwargs):
        ...
        # 使用包装器避免在保存 checkpoint 时序列化失败
        self.tokenizer = _NoSaveTokenizerWrapper(self.get_tokenizer())
