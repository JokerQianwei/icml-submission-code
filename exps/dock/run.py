...
export HTTPS_PROXY=$http_proxy
PYTHONPATH=. CUDA_VISIBLE_DEVICES=4 python exps/dock/run.py -o parp1 -s 0 -n 3000

export https_proxy=$http_proxy
export HTTPS_PROXY=$http_proxy
PYTHONPATH=. CUDA_VISIBLE_DEVICES=1 python exps/dock/run.py -o fa7 -s 0 -n 3000

export https_proxy=$http_proxy
export HTTPS_PROXY=$http_proxy
PYTHONPATH=. CUDA_VISIBLE_DEVICES=2 python exps/dock/run.py -o 5ht1b -s 0 -n 3000

export https_proxy=$http_proxy
export HTTPS_PROXY=$http_proxy
PYTHONPATH=. CUDA_VISIBLE_DEVICES=3 python exps/dock/run.py -o braf -s 0 -n 3000


export https_proxy=$http_proxy
export HTTPS_PROXY=$http_proxy
PYTHONPATH=. CUDA_VISIBLE_DEVICES=5 python exps/dock/run.py -o jak2 -s 0 -n 3000
"""
import os
import sys
sys.path.append('../..')
...
class f_RAG():
    def __init__(self, base_args):
        ...
        config_path = config_dir / "hparams.yaml"
        self.args = EasyDict(yaml.safe_load(open(config_path)))
        self.args.oracle_name = base_args.oracle_name
        self.args.num_mols = base_args.num_mols
        self.args.seed = base_args.seed

        # resolve injection model path relative to the config file for robustness
        if not os.path.isabs(self.args.injection_model_path):
            self.args.injection_model_path = str((config_dir / self.args.injection_model_path).resolve())
    ...
    def set_initial_population(self):
        vocab_path = Path(__file__).resolve().parents[2] / "vocab" / f"{self.args.oracle_name}.csv"
